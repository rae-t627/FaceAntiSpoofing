...
def resnet18(pretrained=False, **kwargs):
    ...
    # change your path
    model_path = '/home/eeiith/Desktop/Project1/Kaustubh/IVP/SSDG-CVPR2020/pretrained_model/resnet18-5c106cde.pth'
    if pretrained:
...
class GRL(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ...
        ctx.low = 0.0
        ctx.high = 1.0
        ctx.max_iter = 4000  # be same to the max_iter of config.py
        ctx.iter_num += 1
    ...
    def backward(ctx, grad_output):
        ...
        coeff = float(2.0 * (ctx.high - ctx.low) / (1.0 + np.exp(-ctx.alpha * ctx.iter_num / ctx.max_iter))
                         - (ctx.high - ctx.low) + ctx.low)
        return -coeff * grad_output.clone()
